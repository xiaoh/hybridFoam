...
defineTypeNameAndDebug(DurbinStab1, 0);
addToRunTimeSelectionTable(RASModel, DurbinStab1, dictionary);

// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

DurbinStab1::DurbinStab1
(
...
:
    ...
    (
        ...
        CmuKE_*sqr(k_)/(epsilon_ + epsilonSmall_)
    )
...
void DurbinStab1::correct()
{
    ...
    (
        ...
        // - fvm::laplacian(nut_/sigmaEps_ + nu(), epsilon_, "laplacian(epsilon)")  // isotrypic diffusion
      ==
    ...
    if(solveK_)
    {
        tmp<fvScalarMatrix> kEqn
            (
                ...
                //  - fvm::laplacian(nut_/sigmaK_ + nu(), k_, "laplacian(k)")   // isotrypic diffusion
                ==
                G
    ...
    #include "fWallIDurbinStab1.H" // set patch internal f values
    ...
    tmp<fvSymmTensorMatrix> REqn
        (
            ...
            // - fvm::laplacian(nut_ + nu(), R_, "laplacian(R)")  // isotrypic diffusion
    ...
    volSymmTensorField S = dev(symm(fvc::grad(U_)));
    dimensionedScalar s0("s0", S.dimensions(), VSMALL);
    nut_ = 2.0/3.0 * Cmu_ * max( (R_ && S/max(mag(S), s0)) , k0_) * T_;
}
