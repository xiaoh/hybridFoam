...
void Foam::fieldAverage::calcAverages()
{
  if(avgSuppressed_)
    {
      Info << name_ << ": " 
           << "Field averaging time not reached.\n" << endl;
      return;
    }
...
void Foam::fieldAverage::reInitFieldsOnOutput()
{
    restartMeanFields<scalar>(meanScalarFields_);
    restartMeanFields<vector>(meanVectorFields_);
    restartMeanFields<sphericalTensor>(meanSphericalTensorFields_);
    restartMeanFields<symmTensor>(meanSymmTensorFields_);
    restartMeanFields<tensor>(meanTensorFields_);
    // Skip current averaging step (current mean is itself)
    prevTimeIndex_ = static_cast<const fvMesh&>(obr_).time().timeIndex();
    Info<< "flexFieldAverage: after output, restarting averaging at time "
...
void Foam::fieldAverage::writeAveragingProperties() const
{
    ...
    propsDict.add("avgSuppressed", avgSuppressed_);
    propsDict.add("justWrote", justWrote_);
...
void Foam::fieldAverage::readAveragingProperties()
{
    if (cleanRestart_)
    {
        Info<< "flexFieldAverage: User specified cleanRestart.\n" 
    ...
    else
    {
        ...
        {
            Info<< "flexFieldAverage: Averaging property file not found.\n"
                << "Starting averaging at time "
                << obr_.time().timeName() << nl << endl;
            return;
        }
        ...
        Info<< "flexFieldAverage: restarting averaging for fields:" << endl;
        forAll(faItems_, fieldI)
        {
            const word& fieldName = faItems_[fieldI].fieldName();
        ...
        propsDict.readIfPresent("avgSuppressed", avgSuppressed_);
        propsDict.readIfPresent("justWrote", justWrote_);
        Info<< endl;
    }
}
...
{
  if(obr_.time().value() > after_  && avgSuppressed_)
    {
      avgSuppressed_ = false;
      initialize();
      prevTimeIndex_ = static_cast<const fvMesh&>(obr_).time().timeIndex();
      Info << "flexFieldAveraging started now." << endl;
    }

    if (active_)
    {
      if(justWrote_) reInitFieldsOnOutput();
  ...
  if (active_)
    {
        ...
        Info << "\n"
             << "flexFieldAverage: Mean field output at time = " 
             << obr_.time().timeName() 
             << nl << endl;
