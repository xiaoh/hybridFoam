...
defineTypeNameAndDebug(muKEpsilon, 0);
addToRunTimeSelectionTable(muRASModel, muKEpsilon, dictionary);

// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

muKEpsilon::muKEpsilon
(
...
:
  muRASModel(typeName, U, phi, lamTransportModel, 
    ...
    (
        ...
        autoCreateEpsilon("epsilonR", mesh_)
    ),
    nut_
    (
        IOobject
...
tmp<volSymmTensorField> muKEpsilon::R() const
{
...
tmp<volSymmTensorField> muKEpsilon::devReff() const
{
    return tmp<volSymmTensorField>
    (
        new volSymmTensorField
...
tmp<fvVectorMatrix> muKEpsilon::divDevReff(volVectorField& U) const
{
    return
    (
...
void muKEpsilon::correct()
{
  ...
  //- This model is seriously out-of-date 
  //-compared to the Low-Re Launder-Sharma KE. Need change before using.
  void muKEpsilon::enforceFields
    ...
    if(turbRelaxTime < runTime_.deltaT() * thresholdImpose_)
      {
        ...
        epsilon_ *= (1.0 - lesFlag_);
        epsilon_ += (epsilonAvg_ * lesFlag_);
      ...
      {
        // Use relax forcing to set fields
